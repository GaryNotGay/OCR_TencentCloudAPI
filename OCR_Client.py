...
def treeviewClick(event):
    ...
    left_data = return_data + mode_data
    left.set(left_data)
...
def Start():
    ...
    if return_data[0:1] == '1':
        mode_data = ModeQuery(mode)
        left_data = return_data[2:5] + mode_data
        left.set(left_data)
        ...
        if return_data[2:5] == '101':
            UsingKeyError()
        elif return_data[2:5] == '102':
            IdentifyError()
...
def About():
    ...
    os.remove('temp.gif')
    ttk.Label(software_frame, text="OCR Lite 1.0").grid(row=0, column=1, sticky = W)
    ttk.Label(software_frame, text="@Author    :   lijishi").grid(row=1, column=1, sticky = W)
    ttk.Label(software_frame, text="@EditTime  :   Jan 23,2020").grid(row=2, column=1, sticky = W)
...
# window layout
global mei_gif
main_window.title("OCR Lite V1.0")
Icon()
